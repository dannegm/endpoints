...
- [ ] **Generar `catalog.ndjson`** — script que lee el JSON completo del catálogo y produce el archivo curado en formato `[libid, title, authors_csv, published, cover_id]` por línea.
  > ⚠️ **Pendiente de evaluar:** cómo entregar este archivo al sistema en producción. La opción preferida es commitear al repo (~12MB estimado), pero primero hay que confirmar el peso real del archivo generado. Dependiendo del resultado, considerar alternativas como Supabase Storage o un volumen persistente.
